...
namespace SFFJunkie.Astral
{
    ...
    {
        private readonly string _defaultLocations = @"Abu Dhabi,UAE,Asia/Dubai,24°28'N,54°22'E
            ...
            ";
        private readonly Dictionary<string, LocationGroup> _groups = new();
        ...
        public Dictionary<string, LocationGroup> Groups { get => _groups; }

        private void AddDefaultLocations()
        {
            foreach (string line in _defaultLocations.Split("\r\n"))
            {
                ...
                var elems = trimmed.Split(",");
                LocationInfo location;
                location = new(elems[0], elems[1], elems[2], elems[3], elems[4]);
                string groupName = location.TimezoneGroup;

                LocationGroup group;
                if (!_groups.ContainsKey(groupName))
                {
                    group = new LocationGroup(groupName);
        ...
        public LocationInfo? Lookup(string name)
        {
            foreach (LocationGroup group in _groups.Values)
            {
                LocationInfo? info = group.Lookup(name);
                if (info != null)
                {
                    return info;
                }
